    ...
    <div class="container-table" data-theme="">
        <div class="table-headers">
            ...
            <form action="#" class="menu__panel-form" id="formAds">
                <input type="search" placeholder="Buscar Anuncios..." class="menu__panel-form-input" />
        ...
        <div class="table-data">
            ...
            <div class="table-items">
                @if ($ads->isNotEmpty())
                    @foreach ($ads as $ad)
                        <x-table-data number="{{ $ad->id }}" category="{{ $ad->category?->name }}"
                            title="{{ $ad->title }}" user="{{ $ad->user?->name }}" estatus="{{ $ad->is_accepted }}"
                            editIcon="icon-pencil" configIcon="icon-delete" />
                    @endforeach
                @else
